...
Student students[NUM_STUDENTS] = {
  ...
  { {0x93, 0x85, 0xCB, 0x13}, "Ali Hassan",     "Form 3A", "Dorm A, Rm 5",  "+254700000001", LOC_UNKNOWN },
  // Student 2 — Tag: 93:E5:02:29
  { {0x93, 0xE5, 0x02, 0x29}, "Fatima Said",    "Form 3B", "Dorm B, Rm 12", "+254700000002", LOC_UNKNOWN },
  // Student 3 — Tag: 23:9E:C8:13
  { {0x23, 0x9E, 0xC8, 0x13}, "James Ochieng",  "Form 4A", "Dorm A, Rm 8",  "+254700000003", LOC_UNKNOWN },
  // Student 4 — Tag: 53:C3:B7:13
  { {0x53, 0xC3, 0xB7, 0x13}, "Sarah Wanjiku",  "Form 4B", "Dorm C, Rm 3",  "+254700000004", LOC_UNKNOWN },
  // Student 5 — Tag: B3:21:D3:26
  { {0xB3, 0x21, 0xD3, 0x26}, "David Mutua",    "Form 2A", "Dorm B, Rm 7",  "+254700000005", LOC_UNKNOWN }
};
...
int findStudent(byte* uid) {
  for (int i = 0; i < NUM_STUDENTS; i++) {
    if (compareUID(students[i].uid, uid)) return i;
  }
  return -1;
}
...
bool isAdmin(byte* uid) {
  return compareUID(uid, adminUID);
}
...
int countAt(byte loc) {
  int count = 0;
  for (int i = 0; i < NUM_STUDENTS; i++) {
    if (students[i].location == loc) count++;
  }
  return count;
}
...
int countInSchool() {
  ...
  for (int i = 0; i < NUM_STUDENTS; i++) {
    ...
    if (loc != LOC_LEFT && loc != LOC_SNEAKED) count++;
  }
  return count;
}
...
void sendEvent(const char* event, int studentIdx, const char* extra1 = NULL, const char* extra2 = NULL) {
...
void updateLCDDefault() {
  ...
  if (millis() - lastLCDScroll < LCD_SCROLL_MS) return;
  ...
  switch (lcdPage) {
    ...
    case 3: {
      ...
      if (displayStudent >= NUM_STUDENTS) displayStudent = 0;
      
      lcd.setCursor(0, 0);
      lcd.print(students[displayStudent].name);
      lcd.setCursor(0, 1);
      switch (students[displayStudent].location) {
        case LOC_UNKNOWN:   lcd.print("Loc: Unknown");    break;
        case LOC_CLASSROOM: lcd.print("Loc: Classroom");  break;
        case LOC_HOSTEL:    lcd.print("Loc: Hostel");     break;
        case LOC_AT_GATE:   lcd.print("Loc: At Gate");    break;
        case LOC_LEFT:      lcd.print("Loc: Left School");break;
        case LOC_SNEAKED:   lcd.print("Loc: SNEAKED!");    break;
      }
      displayStudent++;
      break;
    }
...
bool isCardPresent(MFRC522* reader) {
  ...
  // Reset the reader for a fresh detection attempt
  reader->PCD_WriteRegister(MFRC522::FIFODataReg, reader->PICC_CMD_WUPA);
...
void handleGateDetection(int studentIdx) {
  if (gateState != GATE_IDLE) return; // Already handling a gate event
    ...
    case GATE_WAITING_APPROVAL: {
      ...
      if (readCard(&rfidClass, uid)) {
        if (isAdmin(uid)) {
          ...
          if (idx >= 0) handleZoneTap(idx, ZONE_CLASS);
        }
      }
      ...
      if (readCard(&rfidHostel, uid)) {
        if (isAdmin(uid)) {
          ...
          int idx = findStudent(uid);
          if (idx >= 0) handleZoneTap(idx, ZONE_HOSTEL);
        }
      }
